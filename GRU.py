...
class GRU(nn.Module):
    def __init__(self, hidden_dim, num_layers, weights_matrix, freeze_weights, bidirectional=True):
        ...
        if bidirectional:
            ...
            self.D = 1
        self.fc1 = nn.Linear(hidden_dim*self.D, 300) # for [h_0,h_1,c_0,c_1] of size [1,1200]
    ...
    def forward(self, x, x_len, device):
        ...
        output_packed, h_n = self.gru(x_packed, h0)
        ...
        if self.bidirectional:
            last_h_timestep = torch.concat((h_n[0], h_n[1]), axis=1)
        else:
            last_h_timestep = h_n[0]
...
if __name__ == '__main__':
    ...
    gru = GRU(hidden_dim=HIDDEN_DIM, num_layers=NUM_LAYERS, weights_matrix=embs,
    ...
    fit(gru, train_loader, validation_loader, criterion, optimizer)
    ...
    #     x = gru(train_features_pad, train_len)
